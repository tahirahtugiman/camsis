			...
			<tr class="ui-color-contents-style-1">
				<td colspan="3">
    				...
    				<?php }else { ?>
								...
								<td colspan="10"><span style="color:red; text-transform: uppercase; font-size:14px;">NO <?php if($this->input->get('ppm') == 1){echo "Completed";}if ($this->input->get('ppm') == 2) {echo "Open";}?> PPM WORK ORDER FOUND FOR <?=date('F', mktime(0, 0, 0, $month, 10))?> <?=$year?></span></td>
							</tr>
							<?php } ?>	
					</table>
					<table class="ui-mobile-table-desk ui-left_mobile" style="color:black;">
		        		...
		        		<?php $numrow++?> 			 
							...
							<?php }else { ?>
								...
								<td colspan="2" class="ui-color-color-color default-NO">NO COMPLAINTS FOUND FOR <?=date('F', mktime(0, 0, 0, $month, 10))?> <?=$year?></td>
							</tr>
							<?php } ?>
					</table>
				</td>
