...
if ($this->input->get('broughtfwd') != ''){
  ...
  $tajuk = 'Work Order Listing Unscheduled by Year';
}
...
<div class="none">
	<?php if ($this->input->get('ex') == ''){?>
		...
		<div id="Instruction" class="pr-printer">
    		<div class="header-pr">Work Order Report Listing By Year</div>
			<button onclick="javascript:myFunction('report_volubyyear?m=<?=$month?>&y=<?=$year?>&ex=&stat=<?=$this->input->get('stat');?>&grp=<?=$this->input->get('grp');?>&wid=1&req=<?=$this->input->get('req');?>&broughtfwd=<?=$this->input->get('broughtfwd')?>&serv=<?=$this->input->get('serv')?>&none=closed');" class="btn-button btn-primary-button">PRINT</button>
			<button type="cancel" class="btn-button btn-primary-button" onclick="location.href = '<?php echo $btp ;?>';">CANCEL</button>
			<?php if (($this->input->get('ex') == '') or ($this->input->get('none') == '')){?>
				<a href="<?php echo base_url();?>index.php/contentcontroller/report_volubyyear?req=<?=$this->input->get('req');?>&m=<?=$this->input->get('m');?>&y=<?=$this->input->get('y');?>&ex=excel&xxx=export&none=close&stat=<?=$this->input->get('stat');?>&resch=<?=$this->input->get('resch');?>&broughtfwd=<?=$this->input->get('broughtfwd');?>&grp=<?=$this->input->get('grp');?>&serv=<?=$this->input->get('serv')?>" style="float:right; margin-right:40px;"><img src="<?php echo base_url();?>images/excel.png" style="width:40px; height:38px; position:absolute;" title="export to excel"></a>
				<!--<a href="<?php echo base_url();?>index.php/contentcontroller/report_volubyyear?req=<?=$this->input->get('req');?>&m=<?=$this->input->get('m');?>&y=<?=$this->input->get('y');?>&ex=excel&none=close&stat=<?=$this->input->get('stat');?>&resch=<?=$this->input->get('resch');?>&broughtfwd=<?=$this->input->get('broughtfwd');?>&pdf=1&grp=<?=$this->input->get('grp');?>&serv=<?=$this->input->get('serv')?>" style="float:right; margin-right:80px;"><img src="<?php echo base_url();?>images/pdf.png" style="width:40px; height:38px; position:absolute;" title="export to pdf"></a>-->
			<?php } ?>
		</div>
	<?php } ?>
						...
						<div class="m-div">
							<table class="rport-header">
								<tr>
									...
									<td colspan="16" valign="top"><?php if ($this->input->get('broughtfwd') != ''){?>Unscheduled Brought Forward Work Order Details<?php }else{ ?>Work Order Report Listing<?php }?> - <?=$year?> - <?php echo $this->session->userdata('usersessn');?>  ( <?php if (($this->input->get('req')) and (($this->input->get('grp') == '2') or ($this->input->get('grp') == '3'))){ echo 'Group'.$this->input->get('grp').','.$tulis; } elseif ($this->input->get('req')){echo $tulis; }elseif ($this->input->get('grp') == ''){ echo 'A1 - Breakdown Maintenance (BM)';}else{ echo 'Group '.$this->input->get('grp');} ?> )</td>
									<?php } ?>
								</tr>
							</table>
					...
					<?php } ?>
								...
								<?php echo ($numrow%2==0) ? '<tr class="ui-color-color-color">' : '<tr>'; ?>
									...
									<?php if ($this->input->get('stat') == 'A') {?>
										...
										<?php } else { ?>
											<td><?= (($row->V_request_type == "A10") || ($row->V_request_type == "A3") || ($row->v_tag_no == $assetone) || ($row->linker != "none")) ? '0' : $row->DiffDate ?></td>
										<?php } ?>
									...
									<?php  } else {?>
										...
										<?php if (($this->input->get('broughtfwd') != '') && ($row->v_location_code != $locationone) && ($row->v_tag_no != $assetone) && ($row->V_request_type != "A3") && ($row->V_request_type != "A10") && ($row->linker == "none")){ ?>
											<td><?=$row->DiffDate?></td>
											<!--<td><?= ($row->DiffDate) ? (($row->DiffDate > cal_days_in_month(CAL_GREGORIAN, $this->input->get('m'), $this->input->get('y'))) ? cal_days_in_month(CAL_GREGORIAN, $this->input->get('m'), $this->input->get('y')) : $row->DiffDate) : '1' ?></td>-->
										<?php } else { ?>
