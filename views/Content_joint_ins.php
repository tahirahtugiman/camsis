...
<div id="Instruction" class="pr-printer">
    ...
    <button onclick="javascript:myFunction('joint_ins?en=<?=$this->input->get('en')?>&hosp=IIUM&loc=<?=$this->input->get('loc')?>&month=<?=$fmonth?>&year=<?=$fyear?>&dept=<?=$this->input->get('dept')?>&none=closed&ex=ex');" class="btn-button btn-primary-button">PRINT</button>
	...
	<a href="<?php echo base_url();?>index.php/contentcontroller/joint_ins?m=&loc=<?=$this->input->get('loc')?>&month=<?=$fmonth?>&year=<?=$fyear?>&ex=excel&none=close&en=<?=$this->input->get('en')?>&dept=<?=$this->input->get('dept')?>&hosp=IIUM" style="float:right; margin-right:40px;"><img src="<?php echo base_url();?>images/excel.png" style="width:40px; height:38px; position:absolute;" title="export to excel"></a>
	<?php } ?>
</div>
<?php } ?>
	...
	<table class="tftable" border="1" style="text-align:center; width:70%;" align="center">
		...
		<tr style="text-align:center;font-weight:bold;">
				...
				<?php foreach ($records as $row): ?>
				  <tr>
				    <td><?php echo anchor ('contentcontroller/joint_ins?en=JISFH&hosp='.$row['hospital_code'].'&month='.$fmonth.'&year='.$fyear,isset($row['hospital_code']) ? $row['hospital_code'] : ''); ?></td>
...
<?php }elseif($this->input->get('en') == 'JISFH'){ ?>
			...
			<table class="rport-header">
		<tr>
			<td colspan="5">Joint Inspection Summary : <?=isset($hosp) ? $hosp : ''?> <?= substr(date('M',mktime(0, 0, 0, $fmonth, 10)),0,3).' '.$year ?> - Housekeeping</td>
		</tr>
	</table>
	<table class="tftable" border="1" style="text-align:center; width:70%;" align="center">
		...
		<tr style="text-align:center;font-weight:bold;">
			...
			<?php 
				...
				<?php foreach ($records as $list): ?>
				  <tr>
				    <td><?php echo anchor ('contentcontroller/joint_ins?en=JISBMI&dept='.$list['Dept_Code'].'&hosp='.$list['hospital_code'].'&month='.$fmonth.'&year='.$fyear,isset($list['v_UserDeptDesc']) ? $list['v_UserDeptDesc'] : ''); ?></td>
					<?php if ($deptlist) { ?>
					...
					<?php 
					$total = $list['Satisfactory'] + $list['Unstatisfactory'];
					?>
					<td><?=$total?></td>
				  </tr>
				<?php endforeach; ?>
...
<?php }elseif($this->input->get('en') == 'JISBMI'){ ?>
			...
			<table class="rport-header">
		<tr>
			<td colspan="5">Joint Inspection Summary Hospital : <?=isset($hosp) ? $hosp : ''?> >> <?=isset($dept) ? $dept : ''?> <?= substr(date('M',mktime(0, 0, 0, $fmonth, 10)),0,3).' '.$year ?> - Housekeeping</td>
		</tr>
	</table>
	<table class="tftable" border="1" style="text-align:center; width:70%;" align="center">
		...
		<tr style="text-align:center;font-weight:bold;">
			...
			<?php 
				  ...
				  <tr>
				    <td><?php echo anchor ('contentcontroller/joint_ins?en=JISDoc&loc='.$loc['V_location_code'].'&dept='.$loc['v_UserDeptCode'].'&hosp='.$loc['V_Hospitalcode'].'&month='.$fmonth.'&year='.$fyear.'&locname='.$loc['v_Location_Name'],isset($loc['v_Location_Name']) ? $loc['v_Location_Name'] : ''); ?></td>
					<td>Unit</td>
					...
					<?php
					$total = $loc['Satisfactory'] + $loc['Unstatisfactory'] + $loc['Not_Applicable']
					?>
					<td><?=$total?></td>
				  </tr>
				  <?php endforeach; ?>
...
<?php }elseif($this->input->get('en') == 'JISDoc'){ ?>
	...
	<table class="tftable" border="1" style="text-align:center; width:70%;" align="center">
				 ...
				 <?php foreach ($record as $ld): ?>
				  <tr>
					<td><?=isset ($ld['Job_Date']) ? date("d-m-Y",strtotime($ld['Job_Date'])) : ''?></td> <!--<?php echo anchor ('contentcontroller/joint_ins?&en=JISJNum&date='.$ld->Job_Date.'&dept='.$ld->Dept_Code.'&hosp='.$ld->hospital_code.'&month='.$fmonth.'&year='.$fyear,isset ($ld->Job_Date) ? date("d-m-Y",strtotime($ld->Job_Date)) : ''); ?>-->
				    <td><?php echo anchor ('contentcontroller/joint_ins?&en=JISJNum&dept='.$ld['Dept_Code'].'&hosp='.$ld['hospital_code'].'&month='.$fmonth.'&year='.$fyear.'&jino='.$ld['ji_no'].'&date='.$ld['Job_Date'],isset($ld['ji_no']) ? $ld['ji_no'] : ''); ?></td>
					<td>Unit</td>
