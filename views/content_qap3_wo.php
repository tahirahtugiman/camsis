		...
		<table class="ui-content-middle-menu-workorder" border="0" height="" width="80%" align="center">
			...
			<tr class="" height="40px" style="color:white;">
				<td class="ui-middle-color assets-headear" colspan="3" style="padding-left:10px;" >
					<b>Work Orders with SIQ <?=date('F', mktime(0, 0, 0, $month, 10))?> <?=$year?></b><br />
					<span style="font-size:14px;"><i>SIQ were generated for the following work orders.</i></span>
				</td>
			</tr>
					...
					<div class="ui-left_web wrap">
						<table class="ui-desk-style-table" style="color:black; background:white; text-align:center;" cellpadding="4" cellspacing="0" width="100%" border="0">	
							...
							<?= $numrow%2==0 ? '<tr class="ui-color-color-color item" id="item-<?php echo $numrow?>">' : '<tr class="item" id="item-<?php echo $numrow?>">'; ?>
								...
								<td >
									<?php echo anchor ('contentcontroller/jobclose?wrk_ord='.$row->work_order_no.'&vppm=5',''.$row->work_order_no.'' ) ?>
								</td>
								<?php } else { ?>
								<td >
									<?php echo anchor ('contentcontroller/jobclose?wrk_ord='.$row->work_order_no.'&wo=6',''.$row->work_order_no.'' ) ?>
								</td>
								<?php } ?>
								<td>
								...
								<td>
									<?=!is_null($row->siqppm_no) ? "<span style=color:red;>".anchor ('contentcontroller/qap3_SIQ_Number_update?ssiq='.$row->siqppm_no.'&m='.$month.'&y='.$year,''."<span style=color:red;>".$row->siqppm_no."</span>".'' ).'<br>'.$StatusPPM."</span>" : '' ?>
								</td>
							</tr>
								<?php $numrow++ ?>
								<?php endforeach;?>
							...
							<tr>
								<td align="center" style="height:200px; background:white;" colspan="6">
