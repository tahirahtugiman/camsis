...
<?php if ($this->input->get('ex') == ''){?>
	...
	<div id="Instruction" class="pr-printer">
		...
		<button onclick="javascript:myFunction('report_vols?from=<?=$from?>&to=<?=$to?>&stat=<?=$this->input->get('stat');?>&resch=<?=$this->input->get('resch');?>&grp=<?=$this->input->get('grp');?>&filby=<?=$this->input->get('filby');?>&none=closed');" class="btn-button btn-primary-button">PRINT</button>
		<button type="cancel" class="btn-button btn-primary-button" onclick="delco();location.href = '<?php echo $btp ;?>';">CANCEL</button>
		<?php if (($this->input->get('ex') == '') or ($this->input->get('none') == '')){?>
			<a href="<?php echo base_url();?>index.php/contentcontroller/report_vols?from=<?=$from?>&to=<?=$to?>&none=close&filby=<?=$filby?>&stat=<?php echo $this->input->get('stat');?>&resch=<?php echo $this->input->get('resch');?>&ex=excel&xxx=export&grp=<?=$this->input->get('grp');?>&btp=<?=$this->input->get('btp');?>" style="float:right; margin-right:40px;"><img src="<?php echo base_url();?>images/excel.png" style="width:40px; height:38px; position:absolute;" title="export to excel"></a>
			<a href="<?php echo base_url();?>index.php/contentcontroller/report_vols?from=<?=$from?>&to=<?=$to?>&pdf=1&filby=<?=$filby?>&stat=<?php echo $this->input->get('stat');?>&resch=<?php echo $this->input->get('resch');?>&grp=<?=$this->input->get('grp');?>" style="float:right; margin-right:80px;"><img src="<?php echo base_url();?>images/pdf.png" style="width:40px; height:38px; position:absolute;" title="export to pdf"></a>
		<?php } ?>
	</div>
<?php } ?>
