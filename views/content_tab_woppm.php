	...
	<td colspan="5">
		...
		<?php switch ($tabber) {
			...
			case "0":
				$tulis = "Work Order";
				$left = '';
				$right = base_url().'index.php/contentcontroller/visitplus?wrk_ord='.$this->input->get('wrk_ord').'&vppm=4';
			break;
			case "4":
				$tulis = "Visit +";
				$left = base_url().'index.php/contentcontroller/wo?wrk_ord='.$this->input->get('wrk_ord').'&vppm=0';
				$right = base_url().'index.php/contentcontroller/personnelinvolved?wrk_ord='.$this->input->get('wrk_ord').'&vppm=5';
			break;
			case "5":
				$tulis = "Personnel Involved";
				$left = base_url().'index.php/contentcontroller/visitplus?wrk_ord='.$this->input->get('wrk_ord').'&vppm=4';
				$right = base_url().'index.php/contentcontroller/tech?wrk_ord='.$this->input->get('wrk_ord').'&vppm=7';
			break;
			// case "3":
			case "7":
				$tulis = "Technical Summary";
				$left = base_url().'index.php/contentcontroller/personnelinvolved?wrk_ord='.$this->input->get('wrk_ord').'&vppm=5';
				$right = base_url().'index.php/contentcontroller/clau?wrk_ord='.$this->input->get('wrk_ord').'&vppm=8';
			break;
			// case "4":
			case "8":
				$tulis = "Clause Summary ";
				$left = base_url().'index.php/contentcontroller/tech?wrk_ord='.$this->input->get('wrk_ord').'&vppm=7';
				$right = '';
			break;
		} ?>
