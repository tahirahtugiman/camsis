...
?>
				...
				<td colspan="5" style="" valign="top" >
					<table class="ui-content-middle-menu-workorder2 ui-landscape" width="100%">
						...
						<tr align="center" <?= ($numrow%2==0) ?  'class="ui-color-color-color"' :  '' ?> >
							...
							<td class="td-desk">
								<a rel="nofollow" title="AM : <?=$s_AM?> ON <?=isset($row->DateApproval) ? date("d/m/Y H:i:s",strtotime($row->DateApproval)) : ''?> &#13;Procument : <?=$s_Proc?> ON <?=isset($row->DateApprovalx) ? date("d/m/Y H:i:s",strtotime($row->DateApprovalx)) : ''?> &#13;Logistic : <?=$s_Log?> ON <?=isset($row->DateApprovalxx) ? date("d/m/Y H:i:s",strtotime($row->DateApprovalxx)) : ''?>">
									<?=isset($stat->Status) ? $stat->Status : ''?>
								</a>
							</td>
									<?php } ?>
								<?php } ?>
						...
						<tr align="center" style="height:200px; background:white;">
							<td colspan="10" class="default-NO">
					...
					<table class="ui-portrait" style="color:black;">
						<tbody style="width: 100%;">
							<?php if ($record){//echo "<pre>";var_export($record);?>
			...
			<tr class="ui-header-new ui-landscape" style="height:5px;">
