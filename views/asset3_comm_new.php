<?php if( $this->uri->slash_segment(2) == 'insert_comm_new/' || $this->uri->slash_segment(2) == 'update_delete_comm/'){ ?>
<html>
<body >
</body>
</html>
...
<?php if($this->input->get('MC') == '1') { ?>
	...
	<tr>
		<td colspan="2"><button type="button" value="Change" onclick="location.href = '<?php echo base_url();?>index.php/Procurement/asset3_comm_new?mrinno=<?php echo $this->input->get('mrinno')?>&act=<?php echo $this->input->get('act')?>&id=<?=isset($insertid)?$insertid : $this->input->get('id')?>&tag=<?=$this->input->get('tag')?>';" >Change</button> <input type="submit" value="Add" onClick="javascript:showmg('<?php echo $this->input->get('mrinno');?>','<?php echo $this->input->get('tag') ?>');"> </td>
	</tr>
<?php }else{ ?>
	<tr>
		<th style="text-align:left;">New MRIN <?=ucwords($this->input->get('tag'))?></th>
	</tr>
	<tr>
		<td>Attachment for <?=$this->input->get('mrinno')?></td>
	</tr>
	<tr>
	...
	<?php } else { ?>
	<form  action="update_delete_comm?mrinno=<?php echo $this->input->get('mrinno')?>&id=<?php echo $this->input->get('id')?>&act=<?=$this->input->get('act')?>&tag=<?=$this->input->get('tag')?>" method="post" id="form" name="form">
	<?php } ?>
	<tr>
	...
	<tr>
		...
		<?php } else { ?>
		<input type="button" value="<?=($this->input->get('act') != 'update') ? 'Delete' : 'Save'?>" onClick="javascript:showupdate('<?php echo $this->input->get('mrinno');?>','<?php echo $this->input->get('tag') ?>');">
		<?php } ?>
...
<?php if ($this->input->get('act') != 'update' && $this->input->get('act') != 'delete'){ ?>
	...
	document.getElementById("form").action = "asset3_comm_new?mrinno=<?php echo $this->input->get('mrinno')?>&id=<?php echo $this->input->get('id')?>&act=<?php echo $this->input->get('act')?>&MC=1&tag=<?=$this->input->get('tag')?>";
	return true;
	}
<?php }else{ ?>
function verifyFile()
	...
	document.getElementById("form").action = "update_delete_comm?mrinno=<?php echo $this->input->get('mrinno')?>&id=<?php echo $this->input->get('id')?>&act=<?=$this->input->get('act')?>"
	return true;
	}
<?php } ?>
		function showmg(mrinno,tag)
		{
			self.close();
			window.opener.fCallLocatioa(mrinno,tag);
			self.close();
		}
		function showupdate(mrinno,tag)
		{
