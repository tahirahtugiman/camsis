...
$filename ="MONTHLY RCM PERFORMANCE REPORT IIUM - ".date('F', mktime(0, 0, 0, $month, 10)) .$year.".xls";
...
<div id="Instruction" class="pr-printer">
    <div class="header-pr">MONTHLY RCM PERFORMANCE REPORT</div>
    <button onclick="javascript:myFunction('report_rcmp?m=<?=$month?>&y=<?=$year?>&none=closed');" class="btn-button btn-primary-button">PRINT</button>
	...
	<a href="<?php echo base_url();?>index.php/contentcontroller/report_rcmp?m=<?=$this->input->get('m');?>&y=<?=$this->input->get('y');?>&stat=<?=$this->input->get('stat');?>&ex=excel&none=close" style="float:right; margin-right:40px;"><img src="<?php echo base_url();?>images/excel.png" style="width:40px; height:38px; position:absolute;" title="export to excel"></a>
	<?php } ?>
</div>
<?php } ?>
...
<div class="m-div">
	<table class="rport-header">
		<tr>
			<td colspan="5">MONTHLY RCM PERFORMANCE REPORT <?= ($records[0]->v_HospitalCode) ? $records[0]->v_HospitalCode : 'NA' ?> - <?=date('F', mktime(0, 0, 0, $month, 10))?> - <?php echo $this->session->userdata('usersessn');?></td>
		</tr>
	</table>
<?php if ($this->input->get('ex') == ''){?>
	<div id="Instruction">
...
<?php } ?>
	<table class="tftable" border="1" style="text-align:center;" align="center">
		<tr>
			<th colspan="13" align="center">All</th>
		</tr>
		<tr >
			<th>Hospital</th>
			<th>WO Generated</th>
...
<?php if (($this->input->get('ex') == '') or ($this->input->get('none') == 'closed')){?>
	<table width="99%" border="0">
		...
		<tr>
			<td width="50%">RCM PERFORMANCE Report - <?=date('F', mktime(0, 0, 0, $month, 10))?><br><i>Computer Generated - CAMSIS</i></td>
